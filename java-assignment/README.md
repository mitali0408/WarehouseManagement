...
Tests rely on Quarkus Dev Services to automatically provision a throwaway Postgres container via
Testcontainers — no manual setup needed on most machines, and this is what CI uses. On some local
machines (certain Docker Engine / Colima combinations), Testcontainers' Docker version-negotiation
can fail outright (e.g. an error like `client version 1.32 is too old`). This is a local
Docker/Testcontainers compatibility issue, not something the project itself works around — if you
hit it, the fix is on your machine's Docker setup (e.g. a different Colima/Docker Engine version
combination), not a project configuration change.

This profile is opt-in only — the default build (no profile) is unaffected and still relies on
standard Dev Services, which is what CI and most local setups should use.
